...
import chess from "@/lib/engine/cchess";
...
export default function Chessboard({
  ...
  useEffect(() => {
    if (state.gameStatus === "check") {
      toast.error(`${state.currentTurn === "red" ? "红方" : "黑方"}被将军！`, {
        icon: "⚠️",
      });
  ...
  const handleCellClick = (position: Position) => {
    ...
    if (selectedPosition) {
      ...
      if (pieceAtPosition && currentTurn === getPieceColor(pieceAtPosition)) {
        ...
        // 验证移动是否合法
        const piece = state.board[selectedPosition.y][
          selectedPosition.x
        ] as Piece;
  ...
  const renderCell = (piece: Piece | null, position: Position) => {
    ...
    const lastMove = chess.getLastMove(state);

    const shouldAnimate = lastMove && isSamePosition(lastMove.to, position);

    const fromPixels =
      shouldAnimate && lastMove.from ? getPixelPosition(lastMove.from) : null;
  ...
  return (
    ...
    >
      {showCoordinates && boardSize.width > 0 && (
        <>
